...
var WebIm = (function () {
    ...
    var Sms_host, Sms_id, Sms_user = false, Sms_online_total = 0;
...
//                    }
    ...
    result.init = function (data) { //{smshost,smsid}
        ...
        var top = ($('body').height() - $('#webimwbox').height()) / 2;
        ...
        socket.on('receivemsg', function (data) {
            ...
            if (data) {
                if (data.result == 'ok') {
                    ...
                    if ($('#im_msgbox_' + data.fcdata.fcid)[0]) {
                        socket.emit('readfc', {id: data.fcdata.id});
    ...
    function writemsg(data, m) { //data={fcid,ztitle,msg,fujian}
        ...
        $('<li id="webim_msg_' + id + date + '"><p></p><div class="info"></div></li>')
            .appendTo('#webim_msg_body_' + id);

        //console.log('webim_msg_' + id + date, data);

        $('#webim_msg_' + id + date).find('p:eq(0)').addClass(m).html(data.ztitle);

        $('#webim_msg_' + id + date).find('div.info:last').html(msg);

//        if (data.alertmsg) {
//            $('#webim_msg_' + id + date).append('<p class="alert">(' + data.alertmsg + ')</p>');
//        }
...
//        }
        ...
        var node = $('#webim_tree').tree('find', id);
            ...
            success: function (data, status) {
                ...
                if (data.filePath == undefined) {
                    ...
                    if (data.filePath == '') {
                        ...
                        var row = {
                            url: data.filePath,
                            name: data.filePath.replace('static/uploads/im/', ''),
                            blank: true
                        };
